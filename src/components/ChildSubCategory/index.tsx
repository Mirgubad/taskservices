...
import styles from "../ChildCategory/child-category.module.css"

interface ChildSubCategoryProps {
...
const ChildSubCategory: React.FC<ChildSubCategoryProps> = ({
    ...
    const handleDelete = (key: number) => {
        setSubChildCategories(() =>
            subChildCategories?.filter((category) => category.key !== key)
        )
    }
    ...
    return (
        <div className={styles.child__category}>
            <div
                ...
                className={`${styles["child__category--item"]} ${subChildCategories.length > 0 ? styles.active : ""
                    }`}
            >
                <input
                    disabled={disabled}
