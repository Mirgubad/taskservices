...
import ChildSubCategory from "../ChildSubCategory"
import styles from "../SubCategory/subCategory.module.css"

const SubCategory = () => {
    const [subChildCategories, setSubChildCategories] = useState<JSX.Element[]>(
        []
    )
    const [disabled, setDisabled] = useState(false)

    const addChildCategory = () => {
        setSubChildCategories((prevCategories) => [
            ...prevCategories,
            <ChildSubCategory
                subChildCategories={subChildCategories}
                setSubChildCategories={setSubChildCategories}
    ...
    return (
        <div className={styles.sub__category}>
            <div
                className={`${styles["sub__category--item"]} ${subChildCategories.length > 0 ? styles.active : ""
                    } `}
            >
                <input disabled={disabled} type="text" placeholder="Category Name" />
                {disabled ? (
                    <div className={styles.action__buttons}>
                        <div onClick={() => addChildCategory()} className={styles.addBtn}>
                            <svg
                ...
                {subChildCategories && (
                    <div
                        style={{
                            paddingTop: subChildCategories.length > 1 ? "4rem" : "2rem",
                        }}
                        className={styles.sub__categories}
                    >
                        {subChildCategories}
                    </div>
                )}
